...
export const Listing: FC = () => {
  const intl = useIntl();
  const navigation = useNavigation<NavigationProps>();
  const { myList, mySet, allList, updateTokens } = useManageTokens();
  const [keyword, setKeyword] = useState<string>('');
  const debouncedKeyword = useDebounce(keyword, 1000);
  const [optionalTokens, setOptionalTokens] = useState<TokenOf[]>([]);
  ...
  useEffect(() => {
    ...
    setOptionalTokens(filtered);
  }, [allList, myList, mySet]);
  ...
  }, [myList, optionalTokens, debouncedKeyword]);
  ...
  const onDelete = useCallback(async () => {
    ...
    onToggle(undefined);
    updateTokens();
  }, [activeAccount, toDeletedToken, updateTokens, onToggle]);

  useFocusEffect(updateTokens);

  const flatListData = useMemo(
    () => (keyword ? searchedTokens : optionalTokens),
    [keyword, searchedTokens, optionalTokens],
  );
  ...
  const renderItem: ListRenderItem<TokenOf> = useCallback(
    ({ item, index }) => (
      ...
      >
        ...
        />
        {mySet.has(item.tokenIdOnNetwork) ? (
          <IconButton
            name="TrashSolid"
            type="plain"
            onPress={() => {
    ...
    [navigation, flatListData.length, mySet, onToggle],
  );
